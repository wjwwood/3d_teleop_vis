...
import roslib
roslib.load_manifest('teleop_client')
import rospy
...
class App:
    ...
    def main(self):
        while not rospy.is_shutdown():
            ...
            linear_axis = -self.check_axis(1)
            if abs(linear_axis) <= 0.005:
                linear_axis = 0
            angular_axis = -self.check_axis(0)
            if abs(angular_axis) <= 0.005:
                angular_axis = 0
            msg = Twist()
            msg.linear.x = linear_axis * 1.5
            msg.angular.z = angular_axis * 0.2
